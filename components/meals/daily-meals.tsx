...
function MealItem({ meal, onDelete }: MealItemProps) {
  ...
  const itemPositions = new Map(meal.foods.map((item) => [item.id, useMotionValue(0)]));
  ...
  const handleDragEnd = (
    ...
    const x = itemPositions.get(itemId);
    if (!x) return;
      ...
      {meal.foods.length === 0 ? (
        ...
        <>
          <div className="space-y-2">
            {meal.foods.map((item) => {
              const x = itemPositions.get(item.id);
              const opacity = useTransform(x || useMotionValue(0), [-100, 0], [1, 0]);
              ...
              return (
                <div key={item.id} className="relative">
                  <motion.div
                    ...
                    style={{
                      background: draggedItemId === item.id ? background : 'transparent',
                      opacity: draggedItemId === item.id ? opacity : 0,
                    }}
                  >
                    <Trash2
                  ...
                  <motion.div
                    ...
                    style={{ x }}
                    className="relative bg-background rounded-lg p-2 touch-pan-y cursor-grab active:cursor-grabbing"
                  >
